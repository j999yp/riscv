...
class j999yp(pluginTemplate):
    ...
    #? cutsom commands
       self.elf2hex = 'elf2hex 4 524288 {0} > data.mem'
       self.signatureaddresses = 'riscv{1}-unknown-elf-readelf --wide --symbols {0} | grep -e "begin_signature" -e "end_signature" | awk \'{{ print $$2 }}\' | awk \'{{print strtonum( "0x" $$1 )}}\' | awk \'{{print $$1/4}}\' | paste -sd " " | awk \'{{print $$1" "$$2+1}}\' | tee range | awk \'{{print "sed -n "$$2","$$1"p {3}"}}\' | bash >> {2}'
       self.cutcomments = 'grep -o \'^[^/]*\' {0} > {1}'
       self.iverilog = './src/compile'
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for j999yp start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          elf2hex = self.elf2hex.format('my.elf')
          signatureaddresses = self.signatureaddresses.format(elf,self.xlen,sig_file,"tmp")
          cutcomments = self.cutcomments.format("res.mem","tmp")
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' ../../../../src/sim/testbench.vvp'
          else:
            simcmd = 'echo "NO RUN"'

          # concatenate all commands that need to be executed within a make-target.   
          execute = '@cd {0}; {1}; {2}; {3}; {4}; {5};'.format(testentry['work_dir'], cmd, elf2hex, simcmd, cutcomments, signatureaddresses)
